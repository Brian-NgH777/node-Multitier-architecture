...
class EmailTemplateWhenStartFocusCentre {
    data: any;
    ...
    renderData() {
        let data = this.data;
        if (!data || !data.Concepts || !data.MainConcepts || isNaN(data.FocusGroupType))
            return ''; 
        let arrayJsonLearnings: any[] =[];
        let focusTypeName = 'excelling';
        ...
        if (data.FocusGroupType === FocusType.Struggliing) {
          focusTypeName = 'struggliing';
          focusTypeString = ` <p>Our teachers have allocated the below exercises to help with ${data.FirstNameStudent}'s foundation.</p>`;
        }
        
        let stringListLearnings = data.Concepts;
        let mainConcept = JSON.parse(data.MainConcepts);
        arrayJsonLearnings = stringListLearnings.split(';').map(function(n) {
          return JSON.parse(n);
        });

        // arrayJsonLearnings = listLearnings.map(function(n) {
        //   return JSON.parse(n);
        // });
      ...
      >
        <tbody>
          <tr>
            ...
            >
            <p>We've found some growth opportunities for ${data.FirstNameStudent}!</p>
            <p>${data.FirstNameStudent} completed the below exercise and seem to be ${focusTypeName} at it.</p>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table
        style="width: 80%;margin-bottom:16px;margin-left:auto;margin-right:auto;background-color: #f7f7f7;"
        cellspacing="0"
        cellpadding="5"
      >
        <tbody>
          ...
          <tr>
            <td style="height: auto;">
              ...
              >
                <tbody>
                  <tr>
                    <td style="background-color: #1691ed;text-align: center; color: #ffffff;">
                      <table
                        class="callout"
                      ...
                      >
                        <tbody>
                          <tr>
                            <td style=" width: 10%; text-align: center; vertical-align: middle; background-color: #fff">
                              <img
                                src="https://gameasset.s3.amazonaws.com/EmailTemplate/Assessment/${mainConcept.topicName}.png"
                                style="width: 120px; "
                              />
                            </td>
                            <td
    ...
    if(arrayJsonLearnings.length > 0) {

      for(let i=0; i < arrayJsonLearnings.length; i++) {
        let item = arrayJsonLearnings[i];
      str +=` 
          <table
            style="width: 80%;margin-bottom:16px;margin-left:auto;margin-right:auto;background-color: #f7f7f7;"
            cellspacing="0"
                        ...
                        >
                          <tbody>
                            <tr>
                              <td style=" width: 10%; text-align: center; vertical-align: middle; background-color: #fff">
                                <img
                                  src="https://gameasset.s3.amazonaws.com/EmailTemplate/Assessment/${item.topicName}.png"
                                  style="width: 120px; "
                                />
                              </td>
                              <td
...
Object.seal(EmailTemplateWhenStartFocusCentre);
export default EmailTemplateWhenStartFocusCentre; 